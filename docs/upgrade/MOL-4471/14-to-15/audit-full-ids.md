...
| GHSA-52v5-jr5w-gjxr | high | sigstore | sigstore's `certificateOIDs` verification constraints are silently dropped and never enforced | `<=4.1.0` |
...
| GHSA-73wf-gq98-2v4g | high | browserslist | Browserslist: Uncaught crash / prototype write via untrusted browserslist-stats.json custom stats (normalizeStats) | `<=4.28.6` |
...
| GHSA-c83g-rgw3-j3cx | high | browserslist | Browserslist: Unbounded memory growth (no cache eviction) via distinct query results, leading to eventual OOM | `<=4.28.6` |
...
Totals: 81 (critical 7, high 50, moderate 17, low 7, info 0); unique advisory ids: 81
